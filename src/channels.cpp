...
bool Server::isNickonchan(string &nick, string &chan)
{
...
std::vector<Client *>::iterator Server::clposition(string &nick,
						   string &chan)
{
	for (std::vector<Client *>::iterator it = _m_chans[chan].begin();
	     it != _m_chans[chan].end(); it++) {
...
std::vector<string>::iterator Server::chposition(Client *cl,
						      string &chan)
{
...
{
	if (msg->params.empty()) {
		for (std::map<string, std::vector<Client *> >::iterator it =
			     _m_chans.begin();
		     it != _m_chans.end(); it++)
		...
		for (size_t i = 0; i < msg->params.size(); i++) {
			if (msg->params[i] == "#" || msg->params[i] == "&") {
				i++;
				if (_m_chans.find(msg->params[i - 1] +
						  msg->params[i]) !=
				    _m_chans.end()) {
					send_reply(msg->params[i - 1] +
							   msg->params[i],
						   cl, RPL_NAMREPLY);
					send_reply("", cl, RPL_ENDOFNAMES);
				}
...
void Server::listcmd(Message *msg, Client *cl)
{
	...
	if (msg->params.empty()) {
		...
		for (size_t i = 0; i < msg->params.size(); i++) {
			if (msg->params[i] == "#" || msg->params[i] == "&") {
				i++;
				if (_m_chans.find(msg->params[i]) !=
				    _m_chans.end())
					send_reply(msg->params[i], cl,
						   RPL_LIST);
			}
		}
...
void Server::partcmd(Message *msg, Client *cl)
{
	...
	for (std::vector<string>::iterator it = channels.begin();
	     it != channels.end(); it++) {
