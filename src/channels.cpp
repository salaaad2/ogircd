...
std::vector<Client *>::iterator Server::clposition(std::string &nick,
						   std::string &chan)
{
	for (std::vector<Client *>::iterator it = _m_chans[chan].begin();
	     it != _m_chans[chan].end(); it++) {
...
std::vector<std::string>::iterator Server::chposition(Client *cl,
						      std::string &chan)
{
...
{
	if (msg->params.empty()) {
		for (std::map<std::string, std::vector<Client *> >::iterator it =
			     _m_chans.begin();
		     it != _m_chans.end(); it++)
